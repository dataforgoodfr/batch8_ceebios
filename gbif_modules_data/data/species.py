...
class Species:
    ...
    def info(self):
        return {
            x: self.data[x] for x in ["kingdom", "phylum", "order", "genus", "species"]
        }
    ...
    def search_publications(self, n=5, as_df=True, source="google"):
        extractor = (
            GoogleScholarExtractor() if source == "google" else CoreACExtractor()
        )
    ...
    def fetch_wikipedia_page(self, lang="fr"):
        ...
        if not hasattr(self, "wikipedia_page") or (
            hasattr(self, "wikipedia_page_lang") and self.wikipedia_page_lang != lang
        ):
            ...
            if page.is_species():  # and page.title.lower() == self.name.lower():
                ...
                raise WikipediaError(
                    f"Did not found perfect match on Wikipedia for species {self.name}"
                )
